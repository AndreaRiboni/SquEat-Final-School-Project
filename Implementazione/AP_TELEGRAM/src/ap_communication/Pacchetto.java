...
public class Pacchetto {
     ...
     */
    public static String incapsula(int codice, String... parametri) throws Exception {
        if (codice < 0 || codice >= PARAM_NUM.length) {
            throw new Exception("Codice inesistente.");
        }
        if (PARAM_NUM[codice] != parametri.length) {
            throw new Exception("Numero di parametri errato per il tipo di messaggio selezionato. Riprovare utilizzando " + PARAM_NUM[codice] + " parametri.");
        }
     ...
     * @throws Exception errore
     */
    public static boolean verifica(String[] msg) throws Exception {
        try {
            return (msg.length - 1 == PARAM_NUM[Integer.parseInt(msg[0])]);
        } catch (ArrayIndexOutOfBoundsException | NumberFormatException e) {
            throw new Exception("Codice del messaggio inesistente");
        }
    }
     ...
     */
    @Deprecated
    public static boolean needResponse(String request) throws Exception {
        try {
